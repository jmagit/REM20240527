...
//			tablero.setPieza(3, 3, new PiezaMock(Color.BLANCO), 33);
		...
		void testBuscarPeonesNegros() {
			var result = tablero.buscar(e -> e.getPieza() instanceof Peon && e.getPieza().getColor() == Color.NEGRO);
			assertEquals(8, result.size());
			result.forEach(e -> assertEquals(7, e.getFila()));
		}

		@Test
		void testBuscarReyBlanco() {
			var result = tablero.buscar(e -> e.getPieza() instanceof Rey && e.getPieza().getColor() == Color.BLANCO);
			assertEquals(1, result.size());
			assertEquals(1, result.get(0).getFila());
			assertEquals(5, result.get(0).getColumna());
