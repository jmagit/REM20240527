 ...
 * @version 1.0
 */
public class AjedrezJuego implements Juego<Tablero>, Promocionable {
	private Color turno;
	...
	public void continuar(Tablero tablero, Color turno) {
		this.turno = turno;
	...
	@Override
	public void addPromocionaListener(PromocionEventListener arg) {
		for (int f = 1; f <= 8; f++) {
			for (int c = 1; c <= 8; c++) {
				if(tablero.hayPieza(f, c) && tablero.getPieza(f, c) instanceof Promocionable)
					((Promocionable)tablero.getPieza(f, c)).addPromocionaListener(arg);
			}}
	}

	@Override
	public void removePromocionaListener(PromocionEventListener arg) {
		for (int f = 1; f <= 8; f++) {
			for (int c = 1; c <= 8; c++) {
				if(tablero.hayPieza(f, c) && tablero.getPieza(f, c) instanceof Promocionable)
					((Promocionable)tablero.getPieza(f, c)).removePromocionaListener(arg);
			}}
	}
