 ...
 * @version 1.0
 */
public class Peon extends Pieza implements Promocionable {
	/**
	...
	public void mover(Movimiento movimiento, Tablero tablero) throws JuegoException {
		...
		if (esValido(movimiento, tablero)) {
			if (esPromocion(movimiento)) {
				PromocionEvent e = new PromocionEvent(this, getColor());
				onPromocion.forEach(handler -> handler.promociona(e));
				Pieza nuevaPieza = e.getPieza();
				if (e.isCancel())
