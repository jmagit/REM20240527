...
//	 * @param posición Posición con la fila y la columna en el tablero.
//	 * @return Pieza en el tablero, nulo en caso de que no haya pieza.
//	 */
...
//		// copia.piezas = piezas.clone();
//		for (int i = 0; i < piezas.length; i++) {
//			copia.piezas[i] = piezas[i].clone();
//		}
//		return copia;
//	}
...
//
	...
	public Tablero clone() {
		var copia = new Tablero();
		copia.piezas = (Map<Posicion, Pieza>) ((HashMap<Posicion, Pieza>) piezas).clone();
		return copia;
	}

	private static boolean esValido(int indice) {
	...
	public List<Escaque> buscar(Function<Escaque, Boolean> filtro) {
		List<Escaque> result = new ArrayList<Escaque>();
		for (int f = 1; f <= 8; f++) {
			for (int c = 1; c <= 8; c++) {
				try {
